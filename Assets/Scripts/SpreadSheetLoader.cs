...
namespace Game
{
    ...
    {
        List<SpreadSheetData> _profiles;
        // スプレッドシートからデータを読み込んで、パース処理が完了するまで待つ。
        bool _isLoading = true;

        public IReadOnlyList<SpreadSheetData> Profiles => _profiles;
        public bool IsLoading => _isLoading;

        void Awake()
        {
            // スプレッドシート側に大量のデータがある場合、読み込む数を制限する。
            _profiles = new List<SpreadSheetData>(100);
        }
        ...
        void Parse(string downloadText)
        {
            ...
            foreach (string row in downloadText.Split("\n").Skip(1))
            {
                ...
                SpreadSheetData profile = new SpreadSheetData(
                    cells[0],  // 名前
                    cells[1],  // 表示名
                    cells[2],  // 性別
                    cells[3],  // 年齢
